...
* We already write our first `C` program. Now how to execute that ?. To execute that on the same directory terminal you need to type `cc $fileName.c`. this will creating a new file name `a.out` if there is no error on code that you write.
